...
const login = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!email || !password) {
        return next(new AppError('Please provide email and password!', AuthErrorCode.BAD_REQUEST))
    }

    // 2) Vérification utilisateur et mot de passe
    const user: User | undefined = await db('users').where({ email }).first()
    if (!user || !(await bcrypt.compare(password, user.password))) {
        return next(new AppError('Incorrect email or password', AuthErrorCode.UNAUTHORIZED))
    }

    // 3) Vérification statut utilisateur
    if (!user.active) {
        return next(new AppError('This account is not active! Please contact support.', AuthErrorCode.FORBIDDEN))
    }
    ...
    const cookieExpires = new Date(Date.now() + Number(process.env.JWT_COOKIE_EXPIRES_IN) * 24 * 60 * 60 * 1000)
...
const protect = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) {
        return next(new AppError('You are not logged in!', AuthErrorCode.UNAUTHORIZED))
    }
    ...
    try {
        ...
        if (decoded.exp) {
            ...
            if (timeUntilExpiry <= 0) {
                return next(new AppError('Your session has expired', AuthErrorCode.SESSION_EXPIRED))
            }
        ...
        if (!currentUser) {
            return next(new AppError('User no longer exists', AuthErrorCode.UNAUTHORIZED))
        }

        // 4) Vérification statut utilisateur
        if (!currentUser.active) {
            return next(new AppError('This account has been deactivated', AuthErrorCode.FORBIDDEN))
        }

        // 5) Vérification mise à jour utilisateur
        if (currentUser.updated_at.getTime() > decoded.iat * 1000 + 10000) {
            return next(
                new AppError('User account has been updated. Please log in again', AuthErrorCode.SESSION_EXPIRED),
            )
        }
        ...
        return next(new AppError('Invalid token', AuthErrorCode.UNAUTHORIZED))
    }
})
...
const changePassword = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const updatedUser = await db('users')
        .where({ id })
        .first('id', 'username', 'email', 'phone', 'department', 'localisation', 'profile', 'created_at')
...
const getCurrentUser = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user) {
        return next(new AppError('User not found', AuthErrorCode.NOT_FOUND))
    }
